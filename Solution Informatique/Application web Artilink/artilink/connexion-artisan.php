...
if (isset($_POST['connecter'])) {
    if (!empty($_POST['num_tel']) && !empty($_POST['mdp'])) {
        ...
        if ($select_artisan->rowCount() > 0 && password_verify($mdp, $row['mdp'])) { // Utilisation de password_verify
            $_SESSION['user_id'] = $row;
            header("Location:accueil-artisan.php");
            exit;
        } else {
 ...
 <?php include 'header.php'; ?>
    ...
    <div class="container">
        ...
        <form action="" method="POST">
            ...
            <p>Vous n'avez pas de compte? <a href="inscription-artisan.php">Inscrivez-vous ici</a>.</p>
        </div>
        <div class="form-group signup-link">
            <p>Vous n'etes pas Artisan ? <a href="connexion-client.php">Connectez-vous ici</a>.</p>
        </div>
        </form>
    </div>
